...
Updated to use the new Tasks API (solutions API is deprecated in Python 3.13+)
"""
...
class FaceDetectionResult:
    ...
    landmarks: np.ndarray  # Shape: (478, 3) for FaceLandmarker
...
class FaceDetector:
    """
    Face detection and landmark extraction using MediaPipe FaceLandmarker Tasks API.
    Provides 478 facial landmarks for detailed face analysis.
    """
    ...
    def detect(self, image: np.ndarray) -> Optional[FaceDetectionResult]:
        ...
        return FaceDetectionResult(
            landmarks=landmarks,
            bbox=bbox,
            confidence=0.95,  # FaceLandmarker doesn't expose confidence directly
    ...
    def extract_roi(self, image: np.ndarray, landmarks: np.ndarray, 
        ...
        Returns:
            Cropped ROI image
        """
        # Filter indices that exist in landmarks array
        valid_indices = [i for i in landmark_indices if i < len(landmarks)]
        if not valid_indices:
            return image
            
        # Get points for the specified landmarks
        points = landmarks[valid_indices, :2].astype(np.int32)
    ...
    def get_face_measurements(self, landmarks: np.ndarray) -> Dict[str, float]:
        ...
        if self.JAW_LEFT < len(landmarks) and self.JAW_RIGHT < len(landmarks) and self.FACE_BOTTOM < len(landmarks):
