...
import { analyzeFace, type AnalysisResult } from '../services';
...
const CameraScreen = ({ navigation }: any) => {
    ...
    const analyzePhoto = async () => {
        ...
        try {
            // Call real API
            const result = await analyzeFace(capturedImage);

            if (result.success && result.data) {
                // Navigate with real API results
                navigation.navigate('Results', {
                    imageUri: capturedImage,
                    analysisResults: {
                        skinTone: {
                            type: result.data.skin_tone.fitzpatrick_type,
                            undertone: result.data.skin_tone.undertone,
                            hex: result.data.skin_tone.hex_color
                        },
                        faceShape: {
                            shape: result.data.face_shape.shape,
                            confidence: result.data.face_shape.confidence
                        },
                        hairCoverage: {
                            level: result.data.hair_coverage.level,
                            percentage: result.data.hair_coverage.confidence * 100
                        },
                        colorSeason: result.data.color_season || 'spring'
                    }
                });
            } else {
        ...
        return (
            <View style={styles.container}>
                ...
                <BlurView intensity={80} tint="dark" style={styles.previewControls}>
                    <TouchableOpacity style={styles.retakeButton} onPress={retake} disabled={isAnalyzing}>
                        <RotateCcw size={20} color={isAnalyzing ? colors.text.muted : colors.text.primary} style={{ marginRight: 8 }} />
                        <Text style={[styles.buttonText, isAnalyzing && { color: colors.text.muted }]}>Retake</Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={styles.analyzeButton} onPress={analyzePhoto} disabled={isAnalyzing}>
                        {isAnalyzing ? (
                            <>
